...
- **全球宏观/地缘 digest（macro_digest）**：外围（中东/俄乌地缘、大宗商品、美联储、汇率、外围股市）对 A股有指向性，但原来只混在通用快讯里、AI 抓不住。现 `llm.macro_digest`(**v4-flash**) 据 `ds.eastmoney_global_news`(东财7×24全球) + `ds.cls_telegraph`(财联社) 提炼 `{points(要点), sector_map(→利好/利空哪些A股板块), bias(外围倾向)}`；`app._macro_block()` 拼块**前置注入 5 个 AI**（与 `_tier_block` 并列）；`ai_cache` kind=`macro` **当日 6h 缓存**（首次≈5s，之后 0s）。**传导由 AI 推理**（喂全球要点 + 个股板块/概念），不硬编「事件→板块」映射表。实测能自动产出「中东紧张→利好石油开采/军工，利空航空/化工下游」。**后续(TODO C)**：叠加大宗商品/美元/美股 数值指标，见 `plan/BACKLOG.md`。
