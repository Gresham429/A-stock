...
| `llm.py` | DeepSeek 集成：`daily_recommendation` / `market_screen` / `position_advice` / `entry_advice`(深度入场分析) / `market_overview` / `structure_note`(v4-flash)；温度默认 0.15 |
...
| `rules_store.py` | 交易规则库（SQLite `data/rules.db`，67 条蒸馏自 PA_Agent，12 类，带场景标签，注入 AI）；`for_ai` 注入带 `[R{id}]` 供引用，`active_rule_map` 供校验；`templates/prompts/` 存归档提示词 |
