...
| `tests/test_agent_gates.py` | agent 三道门 + 教训闭集 + 挂单契约（10 例，离线）。**因「试跑污染幂等门」「非交易时段白烧 LLM」等 bug 而生** |
| `tests/test_factor_lab.py` | 因子方向机制（6 例，离线）：显著性门控 / 近期优先 / 过期判定扣结构性滞后 / 留痕只记拐点 / 惰性刷新 / 分位锚点 |
| `store.py` | 自选股持久化 |
...
- **因子回测：`_pa_score` 的权重不再是拍的（factor_lab，2026-07-16）**。`GET /api/factors`、`/api/factors/rolling`、`POST /api/factors/backtest`。
  ...
  - **🔄 IC 自动刷新（2026-07-16）**：`refresh_if_stale()` 在 `_universe_boot` 里惰性调用——**过期才重跑**(14s)，不做增量（增量要处理「哪天该补算」的边界，复杂度不值 14 秒）。
  - **⚠️ IC 天然滞后 `max(HORIZONS)=20` 个交易日**：今日的因子值要等 20 天后才知道未来 20 日收益，故**最新可算的 IC 永远是 `today−20`**。`is_stale()` 必须扣掉这段结构性滞后（≈30 自然日），否则会永远判为过期、每次启动都重跑。**「落后一个月」是正常的，不是数据陈旧。**
  - **方向留痕 `direction_log`**：只在**变化时**记（表里只留拐点）。`flip_rate()` 监控翻转频率——**翻得越勤越不可信**。
  - **不加迟滞（有证据的克制）**：实测 2024-02~2026-06 共 505 个交易日、每日滚动，vol/cum20 各翻 **1 次**、range_pos 翻 **2 次** → `|t|>2 + 60日窗口` 本身已足够稳。加迟滞是**没证据的复杂度**。`flip_rate()` 用于持续监控这个前提是否仍成立。
...
- **Agent 模拟交易 + 失败归因（agent_store/agent_loop，2026-07-16）**：`POST /api/agents` 建 agent（自动配套 paper 账户）、`POST /api/agents/<id>/run`(`{dry:true}` 只决策不下单)、`POST /api/agents/run_all`(后台线程，多档位/同档多账户并行实验)、`GET /api/agents`、`GET /api/agents/<id>/runs`。
  ...
  - **测试存档命名规范** `{档位}-{偏好}{决策器}{变体}`，每组只变一个量。已建 12 个：**重复维度** `微型-均衡单A/B/C/D`（同输入同参数只差随机性——**最该先验的：若同档 4 个每天决策完全不同，AI 输出就是噪音，整个体系前提崩塌**）/ **偏好维度** `微型-稳健单A`·`微型-激进单A` / **决策器维度** `微型-均衡辩A/B` / **档位维度** `小型-均衡单A`·`小型-激进单A`·`中型-均衡单A`·`大型-均衡单A`。**每日 16 次 v4-pro（辩论档算 3 次）≈ 4 分钟**（`run_all` 并行 workers=3；串行要 11 分钟）。嫌贵就删辩论档与大档。
  - **`run_all` 并行 workers=3**：LLM 是 I/O 阻塞。不设更高——每 agent 还要打新浪/腾讯取行情（并发高会限流），且 DeepSeek 并发上限未知。各 paper 账户互不相干、写入由各 store 的 `_LOCK` 串行化，故并行安全。
  - **风险偏好（`risk_pref`）曾是死字段**：存在画像里、API 能改、前端能选，但 **`llm.py` 里 0 处、`block_for_ai()` 压根不接收它** → 用户选稳健/均衡/激进 AI 完全看不到。已修：`profile_store.RISK_GUIDE` 把标签**译成具体行为指令**（「激进」两个字 AI 不知道该怎么做，得说清仓位/波动容忍度上具体怎么变，且注明「激进指仓位与选股，不是抗单」）再注入。
  ...
  - **📋 限价挂单制（2026-07-16，取代即时市价成交）**：AI 决策产出 `limit_price` 的**限价委托**（可一次多笔：不同标的或同标的分批不同价位），`agent_store.place()` 挂单 → **不即时成交**；`agent_loop.sweep_orders()` 判定是否触及。**当日有效**（A股规则，隔日未成交 `expired`）。挂单即按限价冻结资金，避免同轮多笔超额占用。
    - **成交判定不作弊**：**当日**走分时（`ds.tencent_minute`，只看挂单时刻之后的点，精确到分钟）；**隔夜**走日K 的 `low`/`high`。两者都是**既成事实**，与条件单日K补判同理，零未来函数。
    - **成交价一律锁 `limit_price`，不用当时更优价** —— 分时只有每分钟收盘、无分钟内高低，乐观假设会系统性高估表现。**宁可少成交，不可自欺**。
    - **`sweep_orders()` 必须在决策之前跑**，否则 AI 会基于未结算的持仓重复下单。**教训基于真正成交的**（`settled["filled"]`），不是刚挂出的委托。
    - `agent_store.pending_stats()` 出成交率——**未成交本身是信息**（挂太保守 = 系统性错过）。
    - **token 预算**：决策提示词含 规则库+教训+档位+成本块 ≈ **8000 字**。DeepSeek 是推理模型，`max_tokens` 覆盖「思考+正文」，太小会思考耗尽、正文空(`finish_reason=length`)。已设 单决策 **8000** / 多空各 **4000** / **裁判 12000**（裁判上下文最长=决策提示词+多空双方论点，曾设 5000 直接炸）。**别调小**。
  ...
  - **启动即跑 + 三道门**：`app._agent_boot()` 在启动后台线程里跑 `run_all(require_open=True)`。
    ...
    ③ **每 agent 每时段桶原子占位**(`agent_store.claim_slot()`) → 同一 (agent, 日, 桶) 只跑一次。
  - **⏰ 时段分桶（2026-07-16）**：`agent_loop.SLOTS` = 早盘(9:30–11:30) / 尾盘(13:00–15:00)。开 app 时跑**当前所在桶**（该桶未跑过的话）→ 采样翻倍（最多 2 轮/天 = 32 次 v4-pro），时点由真实使用决定。
  - **🚫 错过的桶不补 —— 补跑必然引入未来函数**。要在 14:00 重建 10:00 的决策，得重建**整个信息环境**（全市场快照/板块统计/资金流），而 `ds.sina_all_stocks()` **只有当前快照、无 as-of 参数**（重建需逐只拉 4989 次分时≈3min 且必被限流）。任何一处泄漏 = AI 拿 14:00 的答案做 10:00 的题，结论系统性偏乐观且无法自查。**对比**：条件单能用日K补判，是因为「low 跌破止损价」是**既成事实**，不需要重建上下文——两者性质不同，别混。
  - **⚠️ 幂等必须原子，不能靠「查有没有跑过」**：`already_ran` 是「开头查、180 秒后才写复盘」——**两个并发 run_all 会双双挤过去**。实测发生过：10:19:58 与 10:20:08 各起一轮，两轮都跑完(179s/177s)，微型-均衡单A/B 各跑 2 次。已改用 `claims` 表 `PRIMARY KEY (agent_id,date,slot)` 冲突做原子占位；跑失败 `release_slot()` 释放以便重试。
  ...
  - **踩坑**：局部 import 会让「模块级是否已导入」的检查误判——`current_slot()` 里写过 `from zoneinfo import ZoneInfo`，导致给 `agent_loop` 加模块级 `datetime` 导入时被跳过，运行时 `NameError`（只在挂单路径触发，测试没覆盖到）。局部导入已删、统一走模块级。
