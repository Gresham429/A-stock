...
- **交易成本**（`fees.py`，单一事实源）：费率存**画像**（不硬编码）。用户券商**不免 5 元最低**
  ...
  **有无最低佣金比费率本身更重要**：单笔低于分界(=5/费率)时佣金被 5 元顶起。
  ...
  - **费率时点**：用户 2026-07-16 谈下**万2.5**但**未免最低**（1-3 工作日生效）。
    现有那两笔 002602 是**万9** 时买的 → 画像 6 维持 `万9+5元最低`（对这两笔的佣金无差别，均 5 元）。
    万2.5 生效后：若用户想让**新交易**用万2.5，需改画像 6 `commission_rate=0.00025`（保留 min=5）；
    但对其小单规模数值不变。⚠️ **画像目前只存单一费率、无 as-of 快照**——费率随时间变化时
    历史笔与新笔会同用当前费率（小单因触最低而无影响；大单才需要逐笔费率快照，见待办）。
...
- **佣金已谈(2026-07-16)**：用户谈下**万2.5**但券商**不肯免 5 元最低**（1-3 工作日生效）。
  对其单笔~1400 元规模，降率不省钱（万9/万2.5 都触 5 元最低）；**真正有效的是免最低,券商拒了**。
  → 生效后若要新交易走万2.5：改画像 6 `commission_rate=0.00025`（保留 min=5）。用户说**先不动**。
