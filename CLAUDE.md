...
- **AI 链 = 4 角色分析师 fan-out → 裁判 → 文稿**（每场 6 次 LLM）：`run_analysts` 4 角色（**情绪面 / 题材热点 / 龙虎榜游资 / 龙头梯队**，各读一面出研判，`v4-flash`）→ `judge` 收敛成结构化「明日关注点」（`v4-pro`）→ `article` 文稿（`v4-pro`）。前端「分析师视角」区展示 4 份研判。**资金面（板块资金流）缺现成端点、待二期补**（凑成 vibe 的 5 角色）。裁判亦保留「直读硬指标」降级路径（分析师全失败时）。
