...
2026-09-16 晚：子项目 B（三周期选股 + 自选股买卖点 + 观点账本）已合并并部署，服务器上首轮公共三周期
已生成 15 条；`ai_cache.json` 搬进 `data/`（沙箱下根目录写不进，曾让大盘研判每 30 秒重算、一天烧 41 次）。
服务器上站长的自选股为空，自选股买卖点要先在页面加自选股再点刷新。四个子项目里 C（到点提醒）、
A（复盘加个股）、D（浅色与手机适配）未开始。**
