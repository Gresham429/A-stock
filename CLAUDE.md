...
- **交易成本**（`fees.py`，单一事实源）：费率存**画像**（不硬编码）。用户实为**万9 免最低**
  → 1 万单笔往返 0.232% = **保本涨幅**。**有无最低佣金比费率本身更重要**（免最低 → 拆单零惩罚）。
...
- **打电话调佣金**：万9 → 万2.5 是白捡 1.6%/年。**但务必保住免最低**——只降费率却带回
  5 元下限，对分批建仓反而更贵（`fees.py` docstring 有推导）。
