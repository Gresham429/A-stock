...
| **🤖 何时卖** | 单只持仓 → DeepSeek 结合你的成本 + **波动史 + 财报增速 + 近期新闻 + 私域笔记 + 交易规则**，给出卖出/加仓/止损/止盈价位 + 基本面与政策面判断。 |
...
| **📐 规则** | 交易规则库（55 条，蒸馏自 Al Brooks 价格行为体系）：分 10 类展示，可 **增删改 / 启用停用**；启用中的规则注入所有 AI 分析。 |
...
├── rules_store.py      # 交易规则库（SQLite，蒸馏自 PA_Agent，注入 AI）
...
├── .gitignore
├── data/               # news.db / notes.db / rules.db（本地库，gitignore）
