...
一个**本地运行**的 A 股看板：多股对比 + 点击深挖（研报/龙虎榜/解禁/资金流/**多周期波动图**）+ 随时增删自选股 + **持仓盈亏跟踪** + **顶部大盘研判条** + **全市场两级选股** + **DeepSeek AI 每日推荐与买卖时机建议**（结果落盘缓存、带时间戳）+ **近1年新闻/政策库** + **私域笔记** + **交易规则库（价格行为体系，可增删改，注入 AI）** + **每个名词的新手解释**。
...
| **点击深挖** | 右侧抽屉 7 页：概览（指标+情景区间+**财报营收/净利同比**+**近期新闻**+走势）、**波动**（当日分时/5日/30日/60日/90天/近1年 多周期折线，鼠标逐点看 时间·价·涨跌%）、**K线/箱形**（蜡烛图 MA5/MA20+成交量、箱形图价格分布）、研报（评级/EPS/PDF直链）、龙虎榜（席位TOP5+机构标注）、解禁风险、资金流（30日柱状图）。 |
...
| 当日分时 / 多周期波动 | 腾讯 ifzq（分时）+ 新浪（日K/5分钟） | 波动图用 |
...
├── static/app.js       # 前端逻辑（对比/深挖/波动/大盘/持仓/AI+缓存/新闻/笔记/规则/名词）
