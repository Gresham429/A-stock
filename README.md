...
一个**本地运行**的 A 股看板：多股对比 + 点击深挖（研报/龙虎榜/解禁/资金流/**多周期行情图：分时+K线蜡烛**）+ 随时增删自选股 + **持仓盈亏跟踪** + **顶部大盘研判条** + **全市场两级选股** + **DeepSeek AI 每日推荐与买卖时机建议**（结果落盘缓存、带时间戳）+ **近1年新闻/政策库** + **私域笔记** + **交易规则库（价格行为体系，可增删改，注入 AI）** + **每个名词的新手解释**。
...
| **点击深挖** | 右侧抽屉 6 页：概览（指标+情景区间+**财报营收/净利同比**+**近期新闻**+走势）、**行情**（分时/5日 分时折线 + 近1月/近3月/近半年/近1年 **日K蜡烛 MA5/MA20+成交量**，鼠标逐点看 时间/OHLC·涨跌%；分时**交易时段每30s自动刷新**；下附箱形图价格分布）、研报（评级/EPS/PDF直链）、龙虎榜（席位TOP5+机构标注）、解禁风险、资金流（30日柱状图）。 |
...
| **🔍 全市场选股** | 从**全市场 170 龙头候选池**（10 一级 → 48 二级板块）**结合大盘、跨板块两级筛选**：按资金规模筛可负担标的、可下钻到某二级细分，给 6~10 只推荐 + **具体配置方案** + 板块简评。 |
| **🤖 何时卖** | 单只持仓 → DeepSeek 结合你的成本 + **波动史 + 财报增速 + 近期新闻 + 私域笔记 + 交易规则**，给出卖出/加仓/止损/止盈价位 + 基本面与政策面判断（含建议持有周期、依据规则）。 |
...
| **📐 规则** | 交易规则库（67 条，蒸馏自 Al Brooks 价格行为体系）：分 12 类，可 **增删改 / 启用停用**；每条带**场景标签**（本金 小/中/大 × 周期 短线/波段/长线），选「当前场景」灵活启用；生效规则注入所有 AI 分析。 |
...
- **行情图（分时+K线）**：分时/5日为分时折线（腾讯分时/新浪5分钟）；近1月/近3月/近半年/近1年为新浪日K（前复权）蜡烛图，含 MA5/MA20+成交量、悬停看当日 OHLC；下附箱形图展示区间收盘价四分位分布（★=当前价）。分时在交易时段每 30s 自动刷新。
