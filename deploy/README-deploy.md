...
| 风险 | 具体是什么 | 这套方案怎么挡 |
|---|---|---|
| **API key 被烧光** | 每日推荐、全市场选股、`/api/agents/run_all` 这些接口都能无限制触发 DeepSeek。`v4-pro` 是推理模型，一个循环脚本一夜能把余额清零，而且你第二天才发现 | `ratelimit.py`：每人日预算 + 全站日预算 + 最小调用间隔。计数落 sqlite，多 worker 共享同一份；并发时最多多放行「同时在飞的调用数」（十几二十次，已知取舍，见「额度怎么调」）。计费点在真实的 DeepSeek 调用处，不在 HTTP 路由上（见「额度怎么调」） |
...
- 门的判断与记账是两步（先读计数、调用成功后再加一），并发时可能多放行几次，
  上界是同时在飞的调用数（约十几二十次）。这是有意的取舍：改成「预占加失败退还」会在
  进程被杀时永久漏掉额度，比多花几毛钱更接近静默出错。计费口径仍以真实调用为准。
