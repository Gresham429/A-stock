...
> 日期：2026-07-14 ｜ 类型：功能（llm 提示词/schema + 新校验模块 + rules_store + 2 路由 + 前端）
...
- `verify_basis(ai_basis, ctx, injected_rule_ids) -> list`：遍历 AI 的 basis，对每个 ref：
  - `signal`：名在 `SIGNAL_DICT` 里，则后端填 `value`，标 `ok`；否则 `bad`（闭集外）。
  - `rule`：`R{n}` 的 n 在 `injected_rule_ids` 里，则标 `ok` 并附规则标题；否则 `bad`。
  - 返回富化后的 basis（带 value/label/status）。

### `rules_store.py`
- `for_ai()` 每条规则前缀 `[R{id}]`（原来是 `{title}：{content}` 无 ID），加了前缀 AI 才能引用、后端才能校验。
...
- 每条关键结论后渲染 **依据**（默认折叠；点结论/"看依据"展开）：`区间位置=92% · R12结构止损（已核对）` / `对不上` / `AI未给依据`。
...
- 构造校验用例：
  - AI 引用合法信号「区间位置」，后端填 92% 并标已核对；
  - AI 引用「R99」(未注入) 或「玄学指标」(闭集外)，都标「对不上」；
  - AI basis 空，结论标「AI 未给依据」。
- 浏览器：深挖后打开 entry/position 面板，溯源条可见、点开展开；结论依据默认收起、点开出「已核对/对不上」。

## 风险 / 边界

- **AI 不配合结构化**：DeepSeek 可能少给或给错 `basis`，所以有把握才要求填、空则「未给依据」、错则「对不上」，不阻断主结论。
- **闭集要与 prompt 同源**：信号清单从 `SIGNAL_DICT` 生成注入，避免 prompt 写了但校验没有（或反之）。
- **max_tokens**：basis 增加输出长度，token 量以 `llm.py` 为准（推理模型，别调小）。
