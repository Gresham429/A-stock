...
> 日期：2026-07-14 ｜ 类型：功能（新数据源 + llm + 缓存 + 注入 + 显示）
...
数据源走**路线 A：纯 HTTP，避开 mootdx**（用户机器海外，通达信 TCP 连不上）。本环境已实测：东财概念 slist 与公告 np-anotice 均可用。
...
## 风险 / 边界
- 公告/概念偶发空/限流时该段留空、叙事降级（提示"数据不足"），不阻断主分析。
