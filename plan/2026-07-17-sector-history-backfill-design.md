...
1. **抽 `_agg_sector_payload(date, snap, memb)`**：把 `snapshot_daily` 里的按板块聚合循环抽成
   纯函数（live 与回填共用，所以口径不漂移、可离线单测）。`snap` 是 code 到 `{chg_pct,amount,price,name}` 的映射。
2. **`backfill_sector_daily(days=95, workers=10)`**：
   ...
   - 并发逐股 `ds.sina_kline(code, num=days+5, scale=240)`（新浪 getKLineData，不封 IP），
     然后每根 K 算 `chg_pct=(close/prev_close-1)*100`、`amount≈close*volume`。
   - 按日归组，逐日调 `_agg_sector_payload`，再写入。
   - **只填缺失日期**（`have` 集合 + `ON CONFLICT DO NOTHING`），所以不覆盖 live 行、可重复运行。
...
### static/app.js
- 板块面板 `secStatus` 加「回填历史」按钮：POST 后提示后台进行、完成后刷新。
