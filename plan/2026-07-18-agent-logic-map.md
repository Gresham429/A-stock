...
> `2026-07-17-intraday-agent-scheduler-design.md`（调度器）。

## 0. 10 秒心智模型

一支 **20 个 agent 的舰队**，每个绑一个独立**模拟盘账户** + 一个**投资画像**（决定档位/费率）。
交易日的两个**时段桶**（早盘/尾盘），调度器让每个 agent 各跑一次 `run_day`：
**先结算旧仓与在挂委托，再研判大盘，再选股，再决策（LLM，可 single/debate），过确定性风控硬门，挂限价单，复盘记教训**。
...
flowchart TD
  ...
  P4 --> P5[⑤ 选股 _screen_rows 出候选]
  P5 --> P6[⑥ 决策 single/debate<br/>注入链:档位+费率+教训+宏观+regime+战绩+journal]
  P6 --> P7[⑦ 风控 risk_check 确定性硬门]
  P7 --> P8[⑧ 挂限价单 place 挂 pending + 冻结现金]
  P8 --> P9[⑨ 复盘 detect_failures T+0 教训 + log_equity]
```

## 1. 舰队与账户模型

- **agent**（`agent_store.agents`）：`name` + `account_id`（绑定的模拟盘账户）+ `profile_id`（投资画像）+ `decider`（single/debate）+ active。当前 20 个，覆盖从微型到超大 × 稳健/均衡/激进 × single/debate（见 CLAUDE.md）。
...
- **舰队归属（多用户）**：agents / paper / profiles 数据归站长，所有舰队代码路径进 `userctx.as_fleet()`。`app._fleet_route` 包住全部 `/api/agents*`：GET 对登录用户开放（只读），写操作只允许管理员或站长、否则 403，站长未设置则 503；一次舰队运行读站长的规则库与画像，LLM 计费记 `fleet` 桶。
...
`agent_loop.run_day(agent_id, dry_run, force, slot)`。顺序**有依赖、不可乱**：
...
| ⑤ | 选股 | `screening._screen_rows` | 复用用户面选股；agent 永远带 focus（最强板块），方向打分用全池方向 |
...
- **(a) 非交易日**（`run_all`）：`news_store.is_trading_day()`（周末/动态节假日），全舰队短路。`force` 绕过。
- **(b) 交易时段 `require_open`**（`run_all`）：`_market_open()`（**北京时** `Asia/Shanghai`，9:30–11:30 或 13:00–15:00 **含端点**）。非时段则**只 `sweep_conditions` 补判、跳过决策**（省掉每 agent 一次 v4-pro）。
- **(c) `claim_slot` 原子占位**（`run_day` 调 `agent_store.claim_slot`）：`INSERT claims` PK`(agent_id,date,slot)`，冲突则 False。**原子、无「查-写」窗口**——修过两个并发 `run_all` 都溜过去、10:19:58 与 10:20:08 各跑一遍的真 bug。跑失败 `release_slot` 释放、下次可重试；成功则占住 = 当日该桶幂等。

**时段桶（slot）**：`SLOTS` = 早盘 9:30–11:30 / 尾盘 13:00–15:00。11:30–13:00 午休不是桶。`current_slot()` 非时段返回 `""`；off-session 手动/dry 跑得 slot `"手动"`。

**错过不补**：漏掉的桶**永不补跑**——补跑要重建当时的信息环境（全市场快照/板块/资金流），但 `sina_all_stocks()` 只有**当前**快照、无 as-of 参数，任何泄漏，AI 就用 14:00 的数据答 10:00 的题，系统性过度乐观且不可审计。真交易员也补不了。**对比**：条件单/限价单**能**回判（「跌破止损价」「触及限价」是**既成事实**、不需重建上下文）。

**调度器（两种模式）**：本地 `python3 app.py` 时，`__main__` 在站长上下文（`userctx.as_fleet()`）里起 `_agent_scheduler` 守护线程，`_AGENT_TICK_SEC=300`，每 5 分钟一 tick，启动即跑一次再 `while sleep(300)`。服务器上 gunicorn（`wsgi.py`）不执行 `__main__`、不起这个线程；agent 自动跑在 `scheduler.py` 里且默认关（`ASTOCK_AGENT_AUTO=0`，设 1 才以同样节奏在站长上下文探一轮），保留「不开 app 就不炒股」，否则站长只能手动触发。`_agent_tick` **单飞** `_agent_tick_lock`（占用则跳过——一轮舰队 workers=3 要 7–20 min，防重叠堆叠并发被限流）。**两层幂等**：tick 锁防重叠 + `claim_slot` 防同桶顺序重跑（多余 tick 近零成本）。**存在意义**：修「非交易时段开 app 导致当天决策永不触发」——本地任何时刻开 app，每个交易桶到点各跑一次。
...
**两种 decider**（`DECIDERS`）：
- **`single`**（`_single_decider`）：一次 LLM，`max_tokens=8000`（DeepSeek 推理模型，token 覆盖思考+正文，太小会思考耗尽正文空）。
- **`debate`**（`_debate_decider`）：多头/空头**并行**论证（各 `max_tokens=8000`），然后裁判定夺（`max_tokens=12000`，上下文最长）。**单边失败必须炸掉整个辩论**（不许降级成一面之词——那样系统性偏空且无人察觉）。默认不启用，先用 single 拿基线。

**注入链**（`_decide_prompt` 把 `ctx['blocks']` + 大盘 + 候选 + 持仓拼成 prompt）。`ctx['blocks']` = `_agent_blocks`（`ai_blocks`）+ 战绩块 + journal 块：
1. `_tier_block` 本金玩法档（按 account 总资产）
2. `_fee_block` 交易成本（具体费率 + 保本涨幅，按 profile）
3. `_lesson_block(agent_id)` **自己的**教训（个体记忆）
4. `_lesson_block()` **全舰队**教训只读层（市场真理该共享，标签区分「你本账户」vs「全体」，所以不趋同）
...
## 6. 风控硬门（`risk_check`，全确定性、无 LLM）

**卖**：S1 未持有则拒；S2 `shares > sellable`（T+1 锁定）则拒。
...
**分散在别处的硬门**（写文档/改动时别漏）：
- **整手**：`paper_store.order`（买入 `shares%100`）。
- **涨跌停**：`paper_store.order`（涨停买不到/跌停卖不出）。
- **T+1**：双重——`risk_check` S2 + `paper_store` 卖路径 + `_settle` 置 `sellable`（`lock_date<today` 才可卖，当日买入 `lock_date=today`）。
- **VOL_FLOOR/CEIL=15/130**：**选股门**（`screening._pa_score`），不是 risk_check。注意：不对称——选股放行 vol≤130，但 risk_check 拒 vol>120，120–130 的候选会出现却被风控拒。
- **保本涨幅**：**不是硬拒**——只在提示词提示 + 卖出后 `below_breakeven` 教训。

## 7. 订单生命周期（限价单，当日有效）

`place()`（`agent_store`）挂进 `pending` 表 `status='live'`，存 `limit_price/shares/placed_t/reason/`**`snap`**（决策时属性快照，买入才有——**必须挂单时抓**，成交在后续 sweep 循环、`ctx` 已不存在，重取会读到变了的值=未来函数）。挂单**不即时成交**；现金**in-round 冻结**（`run_day` 只扣内存 `ctx['cash']` 防同轮超额占用，paper.db 成交才真扣）。

`sweep_orders`（必在决策前）调 `_touched` 判触及：**当日**用分时（首个 `price≤limit` 买/`≥limit` 卖）；**隔夜**用日K（`low≤limit` 买/`high≥limit` 卖）。**成交价锁 limit、不取更优**（分时只有分钟收盘，假设更优会系统性高估）。成交，调 `paper_store.order`（`market_open=True` 硬编，回判事实非 live）。**买入成交副作用**：自动挂止损 `px×0.90` + `add_entry` 建仓留痕（用挂单时 snap）。**卖出成交**：`cancel_conditions`。**当日有效**：未触及且非当日，`expired` 自动撤单。

## 8. 条件单 / 止损（`sweep_conditions` + `conditions` 表）
...
- **`settle_entries`**「只算不判罪」：每笔 open entry 5/10/20 交易日后算超额；**幂等**（r5 先到、r20 后到都反复扫，20 日档出齐才落 `settled_at`、此后不再扫）。
- **`outcome` 公式**：`forward_returns` 自**成交价**、按**K线根数**数交易日（不数日历日，防跨周末错位）；`bench_returns` 按个股 +h 根那天的**日历日期**对齐基准（签名收 `end_dates` 而非 horizons，所以「两边各数 h 根」的 bug 不可表达；踩过：停牌致超额高估 10 个百分点）；`excess=stock−bench`，任一 None 则 None（不回退绝对收益，否则把 beta 当能力=PITFALLS 红线）。基准 `sh000001`，与判罪线**同基准**（单一事实源）。
- **判罪线**：`excess_dist`（backtest 顺带产出的超额分位分布，`DIST_PCTS=(1,5,10,25,50,75,90,95,99)`）是**唯一合法来源**；`rank_of` 值转分位，**无分布返回 None、绝不猜**（PITFALLS#1）。`LESSON_PCT=10` 底部十分位=判罪线，**纪律参数**（16 万样本是事实，「取底 10%」是选择，需用户认可）。**「超额<0=失败」是错的**：实测 p50(20日)=−0.90%（个股跑输市值加权指数是常态），会判 53% 为失败、教 AI「你几乎总失败」。判 `JUDGE_H=20`（5 日 vs 20 日 33.1% 判反）。
- **冻结分位 P1**：`entries.x20_pctile` 结算这一刻算一次即冻结，战绩块/journal/`_judge_entry` 都**读它、永不重算**，所以同一笔过去交易分位恒定、不随分布刷新漂移（修「记忆总是变动」）。

**两条教训路径**（关键区分，别混）：
1. **`detect_failures`（T+0 确定性）**：扫**真成交**，`bad(factor)` 门（`factor_lab.directions()` sign<0 才记，所以不惩罚 AI 服从自己的 IC 正因子）。记 9 类即时教训：`chase_high`(range_pos>85)、`high_vol_entry`(vol>96)、`against_sector`(板块<-1%)、`below_breakeven`、`cash_exhausted`、`loss_cut_late`、`stale_hold`、`oversize`（+`rule_violation`）。
2. **`_judge_entry`（结果导向）**：20 日结算后，仅超额落**底部十分位**记 `bad_outcome`（个案非规律）。
教训闭集 `LESSON_KINDS` 10 类（`agent_store`）；`add_lesson` UPSERT`(agent_id,kind,code)` **累加 hits 不加行**（表稀疏、永久保留）；`lessons()` 按 hits DESC（最常犯=最该注入）。

## 10. 记忆系统（P1–P3 + regime-view，共享底座「一份 journal、多个视图」）

- **底座 = `agent_store.journal` 表**（append-only、永不回改，所以结构上不「变动」）。每次决策（买/观望）入一行：`regime` tag + `signals` 快照 + `action` + **`summary`=当时写下的理由原话**（买 reason / 观望 skip_reason，**零新增 LLM**）；结算贴 20 日超额 + 冻结分位（`journal_staple_outcome`，一次性幂等）。
...
| 常量 | 值 | 处 | 性质 |
|------|-----|----|------|
| `SLOTS` | 早盘 9:30–11:30 / 尾盘 13:00–15:00 | agent_loop | 纪律 |
| `_AGENT_TICK_SEC` | 300（5min） | app | 工程 |
| `HORIZONS` / `JUDGE_H` | (5,10,20) / 20 | factor_lab | 对齐评估 |
| `BENCH_SYM` | sh000001（上证） | factor_lab | 单一事实源 |
| `LESSON_PCT` | 10（底部十分位判罪） | agent_loop | **纪律·需用户认可** |
| `STOP_LOSS_PCT` | −10（px×0.90） | agent_loop | 纪律·已回测 |
| `LOSS_CUT_PCT` | −12 | agent_loop | 纪律 |
| `STALE_DAYS` | 20 | agent_loop | 纪律 |
| `CHASE_HIGH_POS` | 85 | agent_loop | 教训门·方向已验（近 60 日 IC t=-4.9） |
| `MAX_POS_PCT` / `MIN_CASH_PCT` | 30 / 10 | agent_loop | 纪律·风控硬门 |
| `MAX_VOL` | 120（年化波动上限） | agent_loop | 纪律·风控硬门 |
| `VOL_FLOOR/CEIL` | 15 / 130 | screening | 纪律·选股门 |
| 费率 | 万2.5 + 5元最低 + 印花千0.5(卖) + 过户万0.1 | fees | 存画像 |
