...
状态：已完成（2026-09-16，分支 `multiuser`，commit `98630d3` 到 docs 提交共 10 个，未合并 main）。
机制说明已并入 CLAUDE.md「多用户与部署」节；本文保留为设计与决策记录。与计划的偏差：舰队记账用
独立的 `fleet` 桶而不是站长个人额度；站长解析加了 60 秒惰性 resolver；`_fleet_route` 在站长未设置时
返回 503；review 里判定可延后的项列在 CLAUDE.md「多用户收尾」。
