...
状态：设计已与用户逐节确认，待实施计划。子项目 B（四个子项目 B 选股与账本 / C 到点提醒 / A 复盘加个股 / D 浅色与手机适配 中的第一个）。
...
3. 改口规则：有效期内只在触发事件时允许修改，触发事件四种：止损触发、目标达成、周期到期、新信息推翻。
   模型必须点名是哪一种，否则只能维持。
4. 运行时刻：交易日 16:00 全量（三周期 + 自选股 + 账本核对）；09:05 只刷短线和自选股短线点；页面手动
   按钮随时可跑，计入个人日额度。
...
`data/picks_public.db`（公共数据，system 名下）。两个库同一张表结构：
...
- 公共三周期：全站每天一次，`scheduler.py` 16:00 触发，记 `system`，结果进 `data/picks_public.db`。
- 个人自选股：按账号各跑一次，`with userctx.as_user(uid)`，记该用户名下；服务器上遍历 `auth.list_uids()`，
  本地 `python3 app.py` 只跑当前站长。09:05 同样。
