...
- C 到点提醒：**已落地（2026-09-17）**。用户要求「一个账号多台手机、点位默认用 AI 分析但可以自己改」，
  于是做成罗列式配置：手机按行写（渠道支持 bark / wecom / dingtalk / log），点位按行写
  （`代码 买下 买上 卖下 卖上 止损 备注`，写了就覆盖那一只，其余继续用 AI 的）。
  触发、去重、只看已上线周期的规则见 `alerts.py` 与 CLAUDE.md 关键机制。
  **还没做的事**：用户尚未订阅真实渠道（服务器上只配了 `log`，命中只写服务器日志）。
  2026-09-17 追加：加了 ntfy 渠道（安卓苹果都有、免费、不注册不备案），页面上有「生成 ntfy 主题」
  一键给配置，地址也可以只粘不管渠道（自动识别）；服务器实测能推到 ntfy.sh。
  用户装个 App 订阅主题、点「保存手机」即可，不需要再改代码。
- A 复盘加个股与自选股：**已落地（2026-09-17）**，设计见 `plan/2026-09-17-review-stocks-design.md`。
  板块资金流前 3 个板块的成员 + 连板梯队前 3 + 龙虎榜净买前 2 挑出当日标的（默认 8 只，带理由、
  不打分），各补行情/财务/舆情/资金/板块/龙虎榜，一次 v4-flash 出档案（tagline、基本面、财务、
  舆情、驱动、风险、盯什么），**输出白名单重建、价位字段一律丢弃**（买卖点归 B）。
  公开块进复盘 envelope；自选股那一层按人隔离，落个人目录，公开复盘里不出现。
  复盘页新增「个股线索」与「我的自选股」两张卡，自选股那张有生成按钮。
- D 浅色主题 + 手机适配：**已落地（2026-09-17）**。`static/theme.css` + `static/theme.js` 三个页面共用，
  默认浅色（右上角可切深色，记 localStorage），手机端表格横向滚动、输入框 16px、抽屉铺满、栅格单列。
  验证方式：本地隔离副本起 5001，Playwright 按 390×844 与 1440×900 截图逐张看（项目文档里写了这个流程）。
