...
- E 选股与因子重构（用户 2026-09-17 提出）：设计与口径见
  `plan/2026-09-17-screening-factor-refactor-design.md`（三周期各一套、候选池按市值分三层、
  两段式筛选、两层验收、中长线攒够数据自动上线）。攒数据三步已落地：每日 PE/PB 快照
  （`universe_store.valuation_snapshot`）、财务面板（`fundamentals_store`，每周一轮）、
  资金流（`moneyflow_store`：每交易日 clist 全市场快照 + `sync` 的 120 天历史种子）。
  中长线已在面板 mask（`picks_pipeline.visible_horizons`，门槛 250 个交易日）。
  下一步按设计档第十节：修抽样口径（按市值分层，会碰判罪线，先在离线副本重跑分布）、
  建分层池与名额、每周期因子与打分、候选名单前向超额追踪、门控自动上线。
