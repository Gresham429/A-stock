...
- E 选股与因子重构（用户 2026-09-17 提出）：目标与口径已定，三周期各一套，候选池按流通市值分三层
  （大盘 500 亿以上、中盘 100 到 500 亿、小盘 30 到 100 亿，30 亿以下不纳入）。攒历史的两步已落地：
  每日全市场 PE/PB 快照（`universe_store.valuation_snapshot`，保留 2 年）与财务面板
  （`fundamentals_store`，每周自动一轮）。第三步资金流历史已探明但未写代码：东财 `push2his` 的
  `fflow/daykline` 能给 120 天每日主力净流入，住宅 IP 通、服务器 IP 时通时断（突发请求会被重置连接），
  真要用得配节流分批，且更适合放在本地机器跑。下一步写设计档
  `plan/2026-09-17-screening-factor-refactor-design.md`，定各周期因子清单、名额分配与两层验收口径。
