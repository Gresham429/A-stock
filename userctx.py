...
线程池同理：ex.map 提交的任务拿不到上下文，要用 ctx_map()。

# 舰队站长

agent 舰队全站只有一套，归站长（owner）所有：agents.db / paper.db / 站长的
画像与规则。所有舰队代码路径（跑 agent、读战绩/教训/house-view）都在
as_fleet() 里执行，其他账号自己目录里的 agents.db 永远为空。站长 uid 来源
优先级：环境变量 ASTOCK_FLEET_OWNER，其次 auth.init() 时 set_fleet_uid() 设的
值（users 表里最早的管理员），都没有则为空串；为空时 fleet_uid() 会按 TTL 惰性
重查一次（auth.init() 注册的 resolver），先起服务再建管理员也不用重启。
"""
...
# 是否在舰队代码路径里（as_fleet() 置位）。llm 据此把 LLM 调用记在 ratelimit.FLEET_UID 名下，
# 只受全站预算约束，不占站长个人额度；随 contextvars 一起传进线程。
_fleet_var: contextvars.ContextVar[bool] = contextvars.ContextVar("astock_in_fleet", default=False)
...
_fleet_uid: str = ""   # auth.init() 用 set_fleet_uid() 填；环境变量优先于它
# 站长为空时的惰性重查：auth.init() 注册一个「查最早管理员」的函数，fleet_uid() 在
# _fleet_uid 仍为空时最多每 FLEET_RESOLVE_TTL 秒调一次。这样「先起服务再 adduser」
# 不用重启进程；userctx 本身不碰 sqlite，查询逻辑留在 auth。
_fleet_resolver: Callable[[], str] | None = None
_fleet_resolved_at: float = 0.0
FLEET_RESOLVE_TTL = 60.0


def set_fleet_uid(uid: str) -> None:
    """记下舰队站长 uid（auth.init() 取 users 表里最早的管理员后调用）。

    传空串表示「还没有管理员」，此时 fleet_uid() 只剩环境变量一条路。
    """
    global _fleet_uid
    if uid and not valid_uid(uid):
        raise ValueError(f"非法用户名: {uid!r}")
...
def set_fleet_resolver(fn: Callable[[], str] | None) -> None:
    """注册站长为空时的惰性重查函数（auth.init() 传 _earliest_admin）；传 None 关闭。"""
    global _fleet_resolver, _fleet_resolved_at
    _fleet_resolver = fn
    _fleet_resolved_at = 0.0


def fleet_uid() -> str:
    """舰队站长 uid：环境变量 ASTOCK_FLEET_OWNER 优先，其次 set_fleet_uid() 设过的值，
    仍为空且注册了 resolver 则每 FLEET_RESOLVE_TTL 秒重查一次，都没有返回空串。

    返回空串时，as_fleet() 里访问个人 store 会因 require_uid() 抛 RuntimeError；
    三个只读视图（教训/house-view/regime-view）捕获后返回空块，与「journal 空时空块」一致。
    """
    global _fleet_uid, _fleet_resolved_at
    env = os.environ.get("ASTOCK_FLEET_OWNER", "").strip()
    if env:
        if not valid_uid(env):
            logger.warning("ASTOCK_FLEET_OWNER=%r 不是合法用户名，忽略", env)
        else:
            return env
    if not _fleet_uid and _fleet_resolver is not None \
            and time.time() - _fleet_resolved_at >= FLEET_RESOLVE_TTL:
        _fleet_resolved_at = time.time()
        try:
            found = _fleet_resolver()
        except Exception as e:  # noqa: BLE001 重查失败按「仍未设置」处理，下个 TTL 再试
            logger.debug("站长重查失败: %s", e)
            found = ""
        if found and valid_uid(found):
            _fleet_uid = found
            logger.info("舰队站长已解析为 %s", found)
    return _fleet_uid


def in_fleet() -> bool:
    """当前是否在 as_fleet() 里。"""
    return _fleet_var.get()


@contextlib.contextmanager
def as_fleet() -> Iterator[str]:
    """切到舰队站长上下文，是 as_user(fleet_uid()) 的快捷方式，并置 in_fleet() 标志。"""
    flag = _fleet_var.set(True)
    try:
        with as_user(fleet_uid()) as uid:
            yield uid
    finally:
        _fleet_var.reset(flag)
...
def Thread(target: Callable[..., Any] | None = None, args: tuple = (),
           kwargs: dict | None = None, daemon: bool | None = None,
           name: str | None = None, **rest: Any) -> threading.Thread:
...
def ctx_map(pool: ThreadPoolExecutor, fn: Callable[[Any], Any], items: Any) -> list:
    """带上下文的 pool.map 替身：结果顺序同 items，任一任务异常向外抛。

    语义与 `list(pool.map(fn, items))` 一致，区别只在每个任务都在调用线程
    copy_context() 出来的副本里跑，所以池线程能读到当前用户。agent_loop 的
    两个池（run_all 的 agent 池、debate 的 bull/bear 池）都碰个人库，必须用它。
    每个任务各拷一份上下文：同一个 Context 不能被两个线程同时 run。
    """
...
def open_db(path: str, timeout: int = 10) -> sqlite3.Connection:
    """打开 sqlite 并统一开 WAL。
